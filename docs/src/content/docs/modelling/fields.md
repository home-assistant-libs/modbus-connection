...
A field is a descriptor you place on a `Component`. It owns the codec, which
turns raw register words into a Python value, but holds no per-read state.
Reading the attribute returns `T | None`: the decoded value, or `None` before
the first read or when a sentinel decodes to "no value".
...
applies to every helper. A `string` has no `scale`, for example. The
[field reference](/modbus-connection/modelling/fields-reference/#options-shared-by-the-register-helpers)
lists all of them. The options below need more explanation than a table row.

### Affine scaling

Numeric fields decode as `raw * scale + offset`. Pass `offset` for a device that
reports a shifted value. For example, `gauge(0, 0.1, offset=-100)` decodes a
...
such as one for an absent register and another for an unplugged probe. Pass all
of them:
...
applied. State them as they appear on the wire: `0xF448` rather than `-3000`.
...
A scaled numeric register, such as a 0.1-scaled temperature or a voltage. It is
the one helper where `scale` is a required positional argument.
...
A single raw register word with no scaling, sign handling, or sentinel. Use it
for a status word you decode yourself.
...
- `enum` is an `IntEnum` field. A code with no member decodes to `None`, warned
...
Both helpers pass the enum class to `NumberField(convert=...)`. `convert`
accepts any `Callable[[int], T]`, and an enum class is a callable that raises
`ValueError` for unknown codes. It also accepts a `Mapping[int, T]`, where a
missing key means the same. Either way an unknown value decodes to `None`,
warned once per distinct value. For a mapping an enum class cannot express,
such as a mapping onto a `StrEnum`, pass the dict inline:
...
`None` without a warning. `boolean` reads the component's register space. For
a coil or discrete input, use the bit fields below.
...
since the last poll by the device itself or by another writer. A value too wide
for the run raises `ValueError` instead of being truncated.
...
- `discrete_input` is a discrete input (FC02). It has no `writable` option
  because discrete inputs are read-only.
...
alongside the field and applied as `10**sf`. This is the SunSpec `sunssf`
convention.
...
as SunSpec's not-implemented `sunssf` value, raises `ValueError`. A write never
guesses a scale. A read decodes the same case to `None`.
...
declare the `sunssf` spec range (-10..10) by default. See the
...
shape the value in a property or construct a field class directly.

Shape the value in a `@property` to compose or transform several fields, or to
unpack packed dates and times. Keep the field private and expose the computed
value, so static typing stays exact:
...
Construct a field class directly when the codec itself is the problem, because
the device packs its words in a way no helper decodes. The helpers on this page
