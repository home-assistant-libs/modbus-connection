...
description: How a component turns its fields into as few Modbus reads as possible, with block pooling, readable address ranges, refused blocks, and the raw register map.
---
...
what shapes those reads: the pooling settings, the device map you can declare
...
Two `Component` class attributes tune this:
...
`max_gap` allows, declare the device's map as described below.
...
`register_ranges` states which addresses the device answers. It describes the
device's map rather than your layout. Most libraries never declare one, because
gap planning already reads a device that serves anything inside its documented
blocks. Declare a map when the device is stricter than that, or when you want
fewer round trips. A map does what `max_gap` cannot:
...
- It forbids a merge. A block never crosses a range boundary, however small
  ...
  The default `max_gap` of 16 bridges a two-register hole otherwise.
...
[`base_offset`](/modbus-connection/modelling/placement/#base_offset-for-the-whole-layout)
...
rather than the whole block, so state an indexed layout's ranges at the
addresses it reads.
...
If some blocks are optional on a device, read them on a separate component. A
missing one then does not fail the rest of the update. The same applies to a
[`ComponentGroup`](/modbus-connection/modelling/component-group/): any failing
block across its pooled members fails the whole group's update.

## Raw diagnostics

`async_read_raw()` returns the device's raw register map alongside the decoded
read, for a diagnostics download or to debug a register layout. It runs the
same reads as `async_update()`, and like it refreshes the fields and fires
listeners. In addition it returns the raw words and bits keyed by absolute
address:
...
The result is keyed by the four Modbus spaces `holding`, `input`, `coil` and
`discrete`. Each is an address-keyed map, addresses ascending. `ComponentGroup`
and `ManualComponent` expose the same method. A group's result is merged
across its members. See
