...
| `require_timeout(seconds)` | Records the requirement on the `required_timeout` attribute for assertions; raises `ValueError` if negative. |
| `require_connect_delay(seconds)` | Records the requirement on the `required_connect_delay` attribute for assertions; raises `ValueError` if negative. |
