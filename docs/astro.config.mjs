    ...
    starlight({
      ...
      plugins: [
        starlightLlmsTxt({
          ...
          description:
            "modbus-connection is a small, backend-neutral Modbus connection abstraction for Python. The top-level package is a pure interface — the ModbusConnection base class, the ModbusUnit Protocol, and shared params dataclasses — with interchangeable tmodbus and pymodbus backends, each exporting a concrete ModbusConnection. An optional modbus_connection.model framework maps a device's registers and coils to typed Python attributes and reads a device in as few Modbus calls as possible.",
          details: [
            ...
            "- A connection is owner-held. Each backend exports a `ModbusConnection` (`modbus_connection.tmodbus.ModbusConnection` / `modbus_connection.pymodbus.ModbusConnection`) built from the shared `ModbusTcpParams` / `ModbusUdpParams` / `ModbusTlsParams` / `ModbusSerialParams` dataclasses. Construction does no I/O — a request connects on demand. It reconnects on demand, read-only requests interrupted by a drop are retried once, writes are never replayed, and after `close()` requests raise `ClientClosedError`. tmodbus covers TCP (socket/rtu), TLS, and serial; pymodbus additionally covers UDP and ASCII-over-TCP.",
